...
//GetViolations ...
func (v *Violation) GetViolations(VehiclePlate string, page, pageSize int, paid, sts, viol int) (vs []*Violation) {
	...
	rows, err := db.Query(sqlStatement, VehiclePlate, pageSize, (page * pageSize))
	if err != nil {
...
//Get ...
func (v *Violation) Get(bID string) (*Violation, error) {

	db := db.GetDBSS()

	stmt := "select * from violations where b_id=$1"

	row := db.QueryRow(stmt, bID)
